...
| 9. 徽章 | 用户端 `/routes` 徽章册 | `GET /badges`、`GET /badges/mine` | `badges`、`user_badges`、`point_records` | 成员4；成员2接页面 | ✅ 九枚种子徽章、自动授予、进度与连续解锁反馈已实现 | 执行最新幂等种子；用户完成目标 | 达标仅授予一次；刷新可见；积分兑换后已得徽章不回退 | 真实 MySQL 完整 11 任务终局证据待持续归档 | 是：后续新增规则类型、字段或迁移 |
