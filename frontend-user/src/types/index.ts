...
export interface Creation extends Entity { user_id: number; template_id: number; culture_item_id: number | null; title: string; prompt: string; input_payload: Record<string, unknown>; output_url: string | null; description: string | null; status: 'PENDING' | 'PROCESSING' | 'SUCCESS' | 'FAILED'; error_message: string | null; retry_count: number; creationId: number; resultUrl: string | null; thumbnailUrl: string | null; templateId: number; cultureItemId: number | null; provider: string; generationMode: 'AI_IMAGE' | 'AI_TEXT_TEMPLATE' | 'MOCK_TEMPLATE' | 'UNKNOWN'; model: string; imageSource: string; fallbackUsed: boolean; visualPrompt: string; tags: string[] }
