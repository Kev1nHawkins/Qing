...
import RouteJourney from '@/views/route/RouteJourney.vue'
...
type ViewName = 'home' | 'cultures' | 'detail' | 'routes' | 'guide' | 'create' | 'community' | 'profile'
const route = useRoute()
const routeViews: Record<string, ViewName> = {
  home: 'home',
  cultures: 'cultures',
  'culture-detail': 'detail',
  routes: 'routes',
  guide: 'guide',
  create: 'create',
  community: 'community',
...
const viewPaths: Record<Exclude<ViewName, 'detail'>, string> = {
  home: '/',
  cultures: '/cultures',
  routes: '/routes',
  guide: '/guide',
...
const view = computed<ViewName>(() => routeViews[String(route.name)] || 'home')
...
const selected = computed<Culture | null>(() => {
  const slug = String(route.params.slug || '')
  return cultures.value.find(item => item.slug === slug) || null
})
...
function navigate(next: ViewName) {
  window.location.assign(next === 'detail' ? '/cultures' : viewPaths[next])
}
function openCulture(item: Culture) {
  window.location.assign(`/cultures/${encodeURIComponent(item.slug)}`)
}
...
<template>
  <div class="m2-app">
    <header class="m2-header">
      <a class="m2-brand" href="/" aria-label="返回探索首页"><span>岭</span><b>岭潮共创<small>LINGNAN · GZHU</small></b><img class="m2-official-logo" :src="gzuOfficialLogo" alt="广州大学" /></a>
      <nav aria-label="主要导航">
        <a :class="{ active: view === 'home' }" href="/">探索</a>
        <a :class="{ active: ['cultures','detail'].includes(view) }" href="/cultures">文化</a>
        <a :class="{ active: view === 'routes' }" href="/routes">校园寻迹</a>
        <a :class="{ active: view === 'guide' }" href="/guide">小棉导览</a>
        <a :class="{ active: view === 'create' }" href="/create">AI 共创</a>
        <a :class="{ active: view === 'community' }" href="/community">社区</a>
        <a :class="{ active: view === 'profile' }" href="/profile">我的</a>
      </nav>
    </header>
    ...
    <main>
      ...
      <template v-else-if="view === 'detail'">
        <PageState :loading="loading" :error="error" :empty="!loading" empty-text="没有找到该文化条目" @retry="loadCultures" />
      </template>

      <template v-else-if="view === 'routes'">
        <RouteJourney @login="navigate('profile')" @create="navigate('create')" />
      </template>
      ...
      <template v-else-if="view === 'community'">
        <section class="m2-page-head"><p class="m2-kicker">COMMUNITY CREATIONS</p><h1>共创社区</h1><p>查看同学们如何把岭南文化、校园地标与青年表达组合成新的文化作品。</p></section>
        <PageState :error="posts.length ? '' : platformError" :empty="!platformError && posts.length === 0" empty-text="社区暂时还没有作品" @retry="loadPlatform" />
        <div v-if="posts.length" class="m2-card-grid">
          <article v-for="post in posts" :key="post.id" class="m2-card">
            <div class="m2-card-photo"><MediaImage :src="post.cover_image_url || visuals.kapok" :alt="post.title" /></div>
            <div class="m2-card-body"><span>社区作品</span><h3>{{ post.title }}</h3><p>{{ post.content }}</p><small>赞 {{ post.like_count }} · 评论 {{ post.comment_count }} · 收藏 {{ post.favorite_count }}</small></div>
          </article>
        </div>
      </template>
...
.m2-header nav a.active {
  color: var(--m2-red);
  font-weight: 700;
}

.m2-header nav a.active::after {
  content: "";
  position: absolute;
  right: 12px;
  bottom: 0;
